...
import os, re, pandas as pd, matplotlib.pyplot as plt, sys, io, warnings
import plotly.express as px
from streamlit_lottie import st_lottie
import requests, datetime, random, threading  # 新增threading导入
import pyperclip  # 新增：用于复制代码功能
...
.exercise-tabs [data-testid="stTab"] {padding:8px 16px!important;}
.exercise-tabs [data-testid="stTab"][aria-selected="true"] {background:#7c3aed!important;color:white!important;border-radius:8px!important;}
</style>
...
# ===================== 新增：练习题核心函数 =====================
def render_code_exercise(chapter_key):
    # ...
    exercises = CODE_EXERCISES.get(chapter_key, [])
    if not exercises:
        st.info(f"暂无{CHAPTER_NAMES.get(chapter_key, chapter_key)}的练习题，敬请期待！")
        return
    # ...
    
    # 初始化章节练习成绩
    if chapter_key not in st.session_state["exercise_scores"]:
        st.session_state["exercise_scores"][chapter_key] = {
            "total": len(exercises),
            "correct": 0,
            "completed": False
        }
    ...
    for idx, exercise in enumerate(exercises):
        exercise_id = f"{chapter_key}_exercise_{idx}"
        st.markdown(f'<div class="exercise-card">', unsafe_allow_html=True)
        
        # 练习题标题和难度
        difficulty_color = {"简单": "#10b981", "中等": "#f59e0b", "较难": "#ef4444"}
        st.markdown(f"""
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
                <h5 style="margin:0;color:#4a044e;">{idx+1}. {exercise['title']}</h5>
                <span style="background:{difficulty_color[exercise['difficulty']]};color:white;padding:2px 8px;border-radius:12px;font-size:12px;">
                    {exercise['difficulty']}
                </span>
            </div>
        """, unsafe_allow_html=True)
        ...
        # 练习题描述
        st.markdown(f"<p style='color:#6b7280;margin-bottom:16px;'>{exercise['description']}</p>", unsafe_allow_html=True)
        
        # 初始化练习答案
        if exercise_id not in st.session_state["exercise_answers"]:
            st.session_state["exercise_answers"][exercise_id] = [""] * len(exercise["blanks"])
        
        # 渲染带填空的代码模板
        st.markdown("<div style='background:#f8f5ff;padding:16px;border-radius:12px;margin-bottom:12px;font-family:Consolas,monospace;'>", unsafe_allow_html=True)
        
        # 拆分代码模板，插入输入框
        code_parts = exercise["code_template"].split("{}")
        answer_inputs = []
        
        for i, part in enumerate(code_parts):
            st.markdown(part, unsafe_allow_html=True)
            if i < len(exercise["blanks"]):
                # 渲染填空输入框
                col_input, col_space = st.columns([2, 10])
                with col_input:
                    user_answer = st.text_input(
                        f"填空{i+1}",
                        value=st.session_state["exercise_answers"][exercise_id][i],
                        key=f"{exercise_id}_blank_{i}",
                        label_visibility="collapsed",
                        placeholder=f"填空{i+1}",
                        args=({"type": "blank-input"},)
                    )
                    answer_inputs.append(user_answer)
                    st.session_state["exercise_answers"][exercise_id][i] = user_answer
        
        st.markdown("</div>", unsafe_allow_html=True)
        
        # 验证答案按钮
        col_verify, col_view, col_empty = st.columns([1, 1, 8])
        with col_verify:
            if st.button(f"验证答案 {idx+1}", key=f"verify_{exercise_id}"):
                # 检查答案
                is_correct = True
                for i, (user_ans, correct_ans) in enumerate(zip(answer_inputs, exercise["blanks"])):
                    if user_ans.strip() != correct_ans:
                        is_correct = False
                        break
                
                # 保存验证结果
                st.session_state["exercise_answers"][exercise_id + "_result"] = is_correct
        ...
        with col_view:
            if st.button(f"查看答案 {idx+1}", key=f"show_{exercise_id}"):
                # 显示正确答案
                correct_code = exercise["code_template"].format(*exercise["blanks"])
                st.markdown(f"""
                    <div style='background:#dcfce7;padding:12px;border-radius:8px;margin-top:8px;'>
                        <p style='color:#166534;margin:0 0 8px 0;font-weight:600;'>正确答案：</p>
                        <pre style='margin:0;background:#f9fafb;padding:8px;border-radius:6px;'>{correct_code}</pre>
                    </div>
                """, unsafe_allow_html=True)
        ...
        if f"{exercise_id}_result" in st.session_state:
            if st.session_state[f"{exercise_id}_result"]:
                st.markdown('<p class="correct-answer">✅ 答案正确</p>', unsafe_allow_html=True)
            else:
                st.markdown('<p class="wrong-answer">❌ 答案错误</p>', unsafe_allow_html=True)
        
        st.markdown('</div>', unsafe_allow_html=True)
        st.divider()
    
    # 章节练习统计
    st.markdown('<div class="card-primary" style="padding:20px;margin-top:20px;">', unsafe_allow_html=True)
    st.markdown("<h4 style='margin:0 0 12px 0;'>📊 章节练习统计</h4>", unsafe_allow_html=True)
    ...
    # 进度条
    if len(exercises) > 0:
        completion_rate = (completed_exercises / len(exercises)) * 100
        accuracy_rate = (correct_exercises / completed_exercises * 100) if completed_exercises > 0 else 0
        
        st.markdown(f"""
            <div style="margin-bottom:12px;">
                <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                    <span>完成进度：</span>
                    <span>{completed_exercises}/{len(exercises)} 题</span>
                </div>
            ...
            <div style="margin-bottom:12px;">
                <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                    <span>正确率：</span>
                    <span>{accuracy_rate:.1f}%</span>
                </div>
                <div class="progress-bar">
                    <div class="progress-fill" style="width:{accuracy_rate}%;background:linear-gradient(90deg,#10b981,#34d399);"></div>
                </div>
            </div>
        """, unsafe_allow_html=True)
        
        # 更新章节成绩
        st.session_state["exercise_scores"][chapter_key]["correct"] = correct_exercises
        st.session_state["exercise_scores"][chapter_key]["completed"] = completed_exercises == len(exercises)
        
        if completed_exercises == len(exercises):
            st.success(f"🎉 已完成所有{len(exercises)}道练习题！正确率：{accuracy_rate:.1f}%")
    
    st.markdown('</div>', unsafe_allow_html=True)# ===========================
# 🎯 代码填空练习（完全仿照你的示例写法 · 全章节可用 + 错题本）
# ===========================
def render_code_exercise(chapter_key):
    st.markdown('<div class="card-white"><h4>🎯 代码填空练习</h4>', unsafe_allow_html=True)

    # 获取当前章节代码（从你原来的 code 变量传入）
    # 直接沿用你原来的逻辑：有代码才显示填空
...
class AIConfig:
    ...
    USER_ID = 'hhq'
    PROMPT_PREFIX = "请仅基于Python大数据分析与挖掘实战的1-8章知识库回答以下问题，保持回答简洁专业："

@st.cache_resource
def get_coze_client():
    try:
        client = Coze(auth=TokenAuth(token=AIConfig.API_TOKEN), base_url=AIConfig.BASE_URL)
        return client
    except Exception as e:
        st.error(f"初始化AI客户端失败：{str(e)}")
        return None
...
def get_coze_answer(prompt: str) -> str:
    if not coze:
        return "⚠️ AI客户端未初始化，请检查配置！"
    
    full_prompt = f"{AIConfig.PROMPT_PREFIX}\n{prompt}"
    try:
        max_retries = 2
        retry_count = 0
        while retry_count < max_retries:
            try:
                chat_poll = coze.chat.create_and_poll(
                    ...
                    additional_messages=[Message.build_user_question_text(full_prompt)],
                    timeout=30
                )
                final_answers = []
                for msg in chat_poll.messages:
                    if msg.role == "assistant" and msg.type == "answer":
                        final_answers.append(msg.content.strip())
                answer = "\n".join(final_answers) if final_answers else "暂无回答"
                return answer
            except Exception as e:
                retry_count += 1
                if retry_count >= max_retries:
                    raise e
                st.warning(f"AI请求失败，正在重试（{retry_count}/{max_retries}）...")
                continue
    except Exception as e:
...
# ====== 新增：代码编辑页 ======
    ...
    st.session_state.custom_code = custom_code  # 实时保存编辑内容
...
# ====== 章节学习页 ======
    ...
    chap_key = st.sidebar.selectbox(
        "选择章节", chapters, 
        index=chapters.index(st.session_state.active_chapter) if st.session_state.active_chapter in chapters else 0,
...
# ====== 学习进度页 ======
    ...
    with c3: st.metric("完成练习题数", completed_exercises)  # 新增练习统计
            ...
            for idx, item in enumerate(filtered_collection):
                ...
                with col_ops[0]:
                    if st.button("🗑️ 取消收藏", key=f"del_col_{idx}"):
                        original_idx = next(i for i, c in enumerate(st.session_state.collection) 
                                          if c["chap"] == item["chap"] and c["file"] == item["file"])
    ...
    if not st.session_state.wrong_book:
        ...
        for idx, wrong in enumerate(st.session_state.wrong_book):
            st.markdown(f"""
                <div style="border-bottom:1px solid #f3e8ff;padding:16px 0;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                        <h5 style="margin:0;color:#7c3aed;">错题 {idx+1}</h5>
...
st.markdown("""
<div class="footer">
    <p>© 2025 视“见”AI - 人工智能可视化学习平台 | 基于Streamlit构建</p>
</div>
""", unsafe_allow_html=True)